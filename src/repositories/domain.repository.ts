...
export class DomainRepository extends BaseRepository<Domain> {
    ...
    async upsertFromRdash(
        ...
        const data = {
            ...
            name: domain.name,
            status: typeof domain.status === 'number' ? String(domain.status) : domain.status,
            ...
            is_premium: domain.is_premium === 1 || domain.is_premium === true,
            is_locked: domain.is_locked === 1 || domain.is_locked === true,
